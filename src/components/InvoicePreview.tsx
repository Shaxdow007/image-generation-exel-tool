...
export function InvoicePreview({ invoice, companyInfo }: InvoicePreviewProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white border-2 border-gray-300 shadow-lg">
      <div className="p-8 text-sm">
        {/* Header */}
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold uppercase tracking-wider">{companyInfo.name || 'VOTRE ENTREPRISE'}</h1>
          <p className="text-sm mt-2">{companyInfo.address || 'Adresse de votre entreprise'}</p>
          <div className="flex justify-center space-x-8 mt-2 text-xs">
            <span>Tel : {companyInfo.phone || 'Téléphone'}</span>
            <span>Fax : {companyInfo.fax || 'Fax'}</span>
          </div>
          <div className="flex justify-center space-x-8 mt-1 text-xs">
            <span>ICE : {companyInfo.ice || 'ICE'}</span>
            <span>R.C : {companyInfo.rc || 'RC'}</span>
          </div>
        </div>
        ...
        <div className="flex justify-between mb-6">
          ...
          <div className="border border-gray-400 p-4 w-5/12">
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div>: {invoice.client.chantier || ''}</div>
              <div><strong>Mode</strong></div>
              <div>: {invoice.client.mode || ''}</div>
            </div>
          </div>
        </div>
        ...
        <table className="w-full border-collapse border border-gray-400 mb-6 text-xs">
          <thead>
            <tr>
              ...
              <th className="border border-gray-400 p-2 text-right bg-gray-100">Montant TTC</th>
            </tr>
          </thead>
          <tbody>
            {invoice.items.map((item) => (
              <tr key={item.id}>
                ...
                <td className="border border-gray-400 p-2 text-right">{item.unitPrice.toFixed(2).replace('.', ',')}</td>
                <td className="border border-gray-400 p-2 text-right">{item.amount.toFixed(2).replace('.', ',')}</td>
              </tr>
            ))}
            {/* Empty rows for better visual spacing */}
            {Array.from({ length: Math.max(0, 8 - invoice.items.length) }).map((_, index) => (
              <tr key={`empty-${index}`}>
        ...
        <div className="flex justify-between items-end">
          <div className="text-xs">
            ...
            <p><strong>{new Date().toLocaleTimeString('fr-FR', { hour12: false })}</strong></p>
            <p className="mt-2">Quantité: <strong>{invoice.totalQuantity.toFixed(2)}</strong></p>
          </div>

          <div className="text-center">
            <div className="border-2 border-blue-500 text-blue-600 font-bold text-2xl p-4 mb-4 transform rotate-12 bg-blue-50">
              Virement
            </div>
          </div>

          <div className="text-right">
            <div className="border border-gray-400 p-3 mb-2 text-center min-w-32">
              <strong>TOTAL</strong>
              <div className="font-bold text-lg">{invoice.subtotal.toFixed(2).replace('.', ',')}</div>
            </div>
            <div className="border border-gray-400 p-3 mb-2 text-center min-w-32">
              <strong>AVANCE</strong>
              <div>{invoice.advance.toFixed(2).replace('.', ',')}****</div>
            </div>
            <div className="border-2 border-gray-600 p-3 text-center bg-gray-100 min-w-32">
              <strong>NET A PAYER</strong>
              <div className="font-bold text-lg">{invoice.netToPay.toFixed(2).replace('.', ',')}</div>
            </div>
          </div>
        </div>
      </div>
